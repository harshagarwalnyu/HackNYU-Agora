...
const { mockSessionStore } = vi.hoisted(() => {
  ...
  const mock = vi.fn(() => ({ ...state }));
  (mock as any).getState = vi.fn(() => state);
  (mock as any).setState = vi.fn((s: Partial<typeof state>) => Object.assign(state, s));
  (mock as any).subscribe = vi.fn();
  return { mockSessionStore: mock };
});
