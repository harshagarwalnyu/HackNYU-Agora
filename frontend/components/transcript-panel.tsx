...
const getMessageBubbleClasses = (from: 'student' | 'tutor') => ({
  container: from === 'student' ? 'justify-end' : 'justify-start',
  bubble: from === 'student'
    ? 'bg-zinc-800 text-white border border-zinc-700'
    : 'bg-zinc-900 text-zinc-100 border border-zinc-800',
  timestamp: from === 'student' ? 'text-zinc-400' : 'text-zinc-500',
  label: from === 'student' ? 'You' : 'Tutor',
});
    ...
    <div className="flex flex-col h-full bg-black">
      ...
      >
        {messages.length === 0 ? (
          ...
          messages.map((msg) => {
            const { container, bubble, timestamp, label } = getMessageBubbleClasses(msg.from as 'student' | 'tutor');
            return (
              <div key={msg.id} className={`flex ${container} animate-fade-in`}>
                <div className={`max-w-[85%] px-5 py-4 rounded-lg ${bubble}`}>
                  <p className="text-[15px] leading-relaxed whitespace-pre-wrap">{msg.text}</p>
                  <p className={`text-xs mt-2.5 ${timestamp}`}>
