...
import { VisualAction } from '@/lib/types';

interface WhiteboardPaneProps {
  onDrawingChange?: (data: unknown) => void;
  isReadOnly?: boolean;
}
...
export const WhiteboardPane = forwardRef<WhiteboardPaneRef, WhiteboardPaneProps>(
  ({ onDrawingChange, isReadOnly = false }, ref) => {
    ...
    const drawNote = (ctx: CanvasRenderingContext2D | null, note: { text: string; x: number; y: number }) => {
      if (!ctx) return;
