...
import type { WhiteboardPaneRef } from '@/components/whiteboard-pane';
import { useWebSocket } from '@/lib/hooks/use-ws';
import { useMessageStore } from '@/lib/store/messages';
import { useSessionStore } from '@/lib/store/session';

const WhiteboardPane = lazy(() =>
  import('@/components/whiteboard-pane').then((m) => ({ default: m.WhiteboardPane }))
);
const SessionSidebar = lazy(() => import('@/components/session-sidebar').then((m) => ({ default: m.SessionSidebar })));

type OrbState = 'idle' | 'listening' | 'thinking' | 'speaking';

export default function AgoraPage() {
  const whiteboardRef = useRef<WhiteboardPaneRef | null>(null);
    ...
    <main className="flex h-screen w-full bg-zinc-950 overflow-hidden">
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 flex overflow-hidden min-h-0">
          {/* Blackboard (before Whiteboard) - lazy-loaded for smaller initial bundle */}
          <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-zinc-900">
            <Suspense fallback={<div className="flex-1 flex items-center justify-center text-zinc-500">Loading board…</div>}>
